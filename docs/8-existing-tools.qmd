...
<!-- TODO (requested from @sec-classification-detection): tracking tools have no
     treatment anywhere in the book. Chapter 2 introduces the tracking-by-detection
     structure and points readers here for the practical side. A companion to Box 1 would
     be the natural home: TrackMate @ershov_trackmate_2022 as the tool most readers will
     actually open (and the one that lets a modern segmentation model be plugged in as
     the detector), plus the linker options and what to do when the detector is the part
     that is failing @maska_cell_2023. Worth noting for tool selection that segmentation
     quality dominates tracking scores, so effort usually belongs on the detector rather
     than on the linker. -->
