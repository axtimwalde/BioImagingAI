...
::: {.callout-note}
**Note for this section, deferred from [Chapter 2](2-primer.qmd).** The primer introduces foundation models compactly and points here for how language models are built. Three points worth developing:

*Why next-word prediction yields more than next-word prediction.* The training objective sounds trivial, but it cannot be met from surface statistics: continuing a sentence requires syntax, and often the fact being referred to. Trained at sufficient scale, the model therefore comes to represent much of the structure that generated its corpus, without ever being told what that structure is. The representation, not the completion, is the product.

*Pre-training buys a representation; post-training makes it usable.* Instruction tuning and reinforcement learning from human feedback [@ouyang_training_2022] are what turn a text completer into something that reliably follows a request. Neither redoes the pre-training, which is why adapting one of these models is so much cheaper than building it.

*In-context learning, stated carefully.* Few-shot prompting did emerge from plain next-token pre-training, plausibly because corpora are full of sequences shaped like a few worked examples followed by an answer; what post-training added was reliability, not the ability itself. Citing this needs a Brown et al. (2020) entry, which `references.bib` does not yet have.
:::
