...
1. **Open Fiji – activate the LOCI update site and restart Fiji.**  
   LOCI tools are required for exporting ROI maps, so enabling this update site ensures the necessary plugins are installed.
...
4. **Start drawing around each object (yes, each one!).**  
   Carefully trace each cell or feature you want to annotate—precision is key to ensure useful training data for deep learning.
...
9. **At the end, you will have one folder with the original images and another for the ROI maps.**  
   This separation makes it easier to organize and use your data with image analysis or deep learning pipelines.
:::
