...
| **Fine tuned deep learning models**: Allows you to update or add to a set of learned transformations (initially created by the developer) to define the positions and boundaries of objects that look like yours.  | Cellpose @Pachitariu2022, µSAM @Archit2025 | Medium | Maximum likelihood for very good quality with less human time than manual annotation | High | High if properly represented in the training set | Most computationally intensive. Many available models are not provided in a non-computationalist friendly format. Can be hard to package into workflows.  |
